...
import { getTags } from './logic/rules';
...
export function AppView() {
  ...
  if (screen === 'taste') return <main className="shell detail"><button className="back" onClick={() => setScreen('result')}>← 返回结果</button><p className="eyebrow">CASE / 053</p><h2>我的完整口味说明书</h2>{(['love', 'like', 'maybe', 'no', 'avoid', null] as Answer[]).map(answer => { const label = answer === 'love' ? '无脑吃' : answer === 'like' ? '大部分吃' : answer === 'maybe' ? '偶尔吃' : answer === 'no' ? '不吃但不雷' : answer === 'avoid' ? '雷' : '不了解'; const list = questions.filter(question => (saved.answers[question.id] ?? null) === answer); return <section className="taste-group" key={label}><h3>{label}</h3><p>{list.length ? list.map(question => <span key={question.id}>{question.title}</span>) : '—'}</p></section>; })}</main>;
